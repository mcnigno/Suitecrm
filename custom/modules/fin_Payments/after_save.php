    ...
    class after_save
    {
		public function after_save_method($bean, $event, $arguments)
        {
			if ($bean->raw_amount_c > 0){
				$bean->payments_type_c = 'Income';
                $budget_type = 'Incoming';
                //$bean->notes_c = 'we were here...Incoming'
			} else {
                $bean->payments_type_c = 'Expenses';
                //$bean->notes_c = 'we were here...Expenses'
			...
			if ($bean->load_relationship('aos_invoices_fin_payments_1')) {
				...
				{
					//Fetch related beans
					if ($bean->load_relationship('fin_vendor_invoice_fin_payments_1')){
