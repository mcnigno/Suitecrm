...
EOF;
    ...
    public function action_createActivitiesSpotsData($filepath)
    {
        ...
        SELECT
            ...
            , calls.date_start
            , RTRIM(LTRIM(CONCAT(COALESCE(users.first_name,''),' ',COALESCE(users.last_name,'')))) as assignedUser
            , COALESCE(DATE(calls.date_start),'$this->nullSqlPlaceholder') as dateStartDay
            , COALESCE(TIME(calls.date_start),'$this->nullSqlPlaceholder') as timeStartDay
            , DAYNAME(calls.date_start) as dateCreatedDay
            , CAST(WEEK(calls.date_start) as CHAR(5)) as dateCreatedWeek
            , concat('(',MONTH(calls.date_start),') ',MONTHNAME(calls.date_start)) as dateCreatedMonth
            , COALESCE(QUARTER(calls.date_start),'$this->nullSqlPlaceholder') as dateCreatedQuarter
            , YEAR(calls.date_start) as dateCreatedYear
        FROM calls
        LEFT JOIN users
...
EOF;
        ...
        SELECT
            ...
            , meetings.date_start
            , RTRIM(LTRIM(CONCAT(COALESCE(users.first_name,''),' ',COALESCE(users.last_name,'')))) as assignedUser
            , COALESCE(DATE(meetings.date_start),'$this->nullSqlPlaceholder') as dateStartDay
            , COALESCE(TIME(meetings.date_start),'$this->nullSqlPlaceholder') as timeStartDay
            , DAYNAME(meetings.date_start) as dateCreatedDay
            , CAST(WEEK(meetings.date_start) as CHAR(5)) as dateCreatedWeek
            , concat('(',MONTH(meetings.date_start),') ',MONTHNAME(meetings.date_start)) as dateCreatedMonth
            , COALESCE(QUARTER(meetings.date_start),'$this->nullSqlPlaceholder') as dateCreatedQuarter
            , YEAR(meetings.date_start) as dateCreatedYear
        FROM meetings
        LEFT JOIN users
...
EOF;
        ...
        SELECT
            ...
            , tasks.date_start
            , RTRIM(LTRIM(CONCAT(COALESCE(users.first_name,''),' ',COALESCE(users.last_name,'')))) as assignedUser
            , COALESCE(DATE(tasks.date_start),'$this->nullSqlPlaceholder') as dateStartDay
            , COALESCE(TIME(tasks.date_start),'$this->nullSqlPlaceholder') as timeStartDay
            , DAYNAME(tasks.date_start) as dateCreatedDay
            , CAST(WEEK(tasks.date_start) as CHAR(5)) as dateCreatedWeek
            , concat('(',MONTH(tasks.date_start),') ',MONTHNAME(tasks.date_start)) as dateCreatedMonth
            , COALESCE(QUARTER(tasks.date_start),'$this->nullSqlPlaceholder') as dateCreatedQuarter
            , YEAR(tasks.date_start) as dateCreatedYear
        FROM tasks
        LEFT JOIN users
...
EOF;
        ...
        SELECT
            ...
            , COALESCE(DATE(calls.date_start),'$this->nullSqlPlaceholder') as dateStartDay
            , COALESCE(TIME(calls.date_start),'$this->nullSqlPlaceholder') as timeStartDay
            , DATENAME(weekday,calls.date_start) as dateCreatedDay,
            , CAST(DATEPART(wk,calls.date_start) as CHAR(5)) as dateCreatedWeek,
            '(' + CAST(DATEPART(mm,calls.date_start)as CHAR(12)) + ') ' + DATENAME(month,DATEPART(mm,calls.date_start)) as dateCreatedMonth,
            COALESCE(DATEPART(qq,calls.date_start),'$this->nullSqlPlaceholder') as dateCreatedQuarter,
            CAST(YEAR(calls.date_start) as CHAR(10)) as dateCreatedYear
        FROM calls
        LEFT JOIN users
            ON calls.assigned_user_id = users.id
            AND users.deleted = 0
...
EOF;
        ...
        SELECT
            ...
            , COALESCE(DATE(meetings.date_start),'$this->nullSqlPlaceholder') as dateStartDay
            , COALESCE(TIME(meetings.date_start),'$this->nullSqlPlaceholder') as timeStartDay
            , DATENAME(weekday,meetings.date_start) as dateCreatedDay,
            , CAST(DATEPART(wk,meetings.date_start) as CHAR(5)) as dateCreatedWeek,
            '(' + CAST(DATEPART(mm,meetings.date_start)as CHAR(12)) + ') ' + DATENAME(month,DATEPART(mm,meetings.date_start)) as dateCreatedMonth,
            COALESCE(DATEPART(qq,meetings.date_start),'$this->nullSqlPlaceholder') as dateCreatedQuarter,
            CAST(YEAR(meetings.date_start) as CHAR(10)) as dateCreatedYear
        FROM meetings
        LEFT JOIN users
            ON meetings.assigned_user_id = users.id
            AND users.deleted = 0
...
EOF;
        ...
        SELECT
            ...
            , COALESCE(DATE(tasks.date_start),'$this->nullSqlPlaceholder') as dateStartDay
            , COALESCE(TIME(tasks.date_start),'$this->nullSqlPlaceholder') as timeStartDay,
            DATENAME(weekday,tasks.date_start) as dateCreatedDay,
            CAST(DATEPART(wk,tasks.date_start) as CHAR(5)) as dateCreatedWeek,
            '(' + CAST(DATEPART(mm,tasks.date_start)as CHAR(12)) + ') ' + DATENAME(month,DATEPART(mm,tasks.date_start)) as dateCreatedMonth,
            COALESCE(DATEPART(qq,tasks.date_start),'$this->nullSqlPlaceholder') as dateCreatedQuarter,
            CAST(YEAR(tasks.date_start) as CHAR(10)) as dateCreatedYear
        FROM tasks
        LEFT JOIN users
            ON tasks.assigned_user_id = users.id
            AND users.deleted = 0
...
EOF;
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $x->{$mod_strings['LBL_AN_ACTIVITIES_DATE_START']} = $row['date_start'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_DATE_START_DATE']} = $row['dateStartDay'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_TIME_START_DATE']} = $row['timeStartDay'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_DAY_CREATED']} = $row['dateCreatedDay'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_WEEK_CREATED']} = $row['dateCreatedWeek'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_MONTH_CREATED']} = $row['dateCreatedMonth'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_QUARTER_CREATED']} = $row['dateCreatedQuarter'];
            $x->{$mod_strings['LBL_AN_ACTIVITIES_YEAR_CREATED']} = $row['dateCreatedYear'];
